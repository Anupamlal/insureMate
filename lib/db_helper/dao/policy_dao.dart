...
class PolicyDao {
  ...
  static Future<bool> insert(Policy policy) async {
    final db = await DbHelper.getInstance.getDB();
    int rowsAffected = await db.insert(POLICY_TABLE, policy.toJson());
  ...
  static Future<Policy> getPolicy(String policyNo) async {
    final db = await DbHelper.getInstance.getDB();
    final allPolicies = await db.query(POLICY_TABLE, where: '$COLUMN_POLICY_NO = ?',
      whereArgs: [policyNo],);
  ...
  static Future<bool> updatePolicy(String policyNo, bool isUploaded) async {
    ...
    int rowsAffected = await db.update(POLICY_TABLE, {COLUMN_IS_UPLOADED: isUploaded ? 1 : 0}, where: '$COLUMN_POLICY_NO = ?',
        whereArgs: [policyNo]);
